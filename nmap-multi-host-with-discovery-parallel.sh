...
function port_scan_udp_top() {
    ...
    sudo nmap ${OUT_OF_SCOPE_ARG} --min-hostgroup ${MIN_HOSTGROUP} -Pn --disable-arp-ping -${SCAN_SPEED} -sU --source-port 53 --top-ports ${NUM_TOP_PORTS_UDP} -sC -sV -v -O -oA "${FILE_RESULTS_UDP_TOP_DETAILED}" ${HOSTS} | tee -a "${DIR_RESULTS}/nmap_logs.txt"
}
